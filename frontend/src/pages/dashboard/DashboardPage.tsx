            ...
            <div className="max-w-2xl">
              ...
              <div className="mt-7 flex flex-wrap gap-3">
                <button
                  ...
                  className="group inline-flex h-11 items-center gap-2 rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground shadow-md shadow-primary/20 transition hover:-translate-y-0.5 hover:opacity-95"
                >
                  <UploadCloud size={17} />
                  Upload document
                ...
                <button
                  ...
                  className="inline-flex h-11 items-center gap-2 rounded-xl border border-border bg-background/60 px-4 text-sm font-semibold text-foreground transition hover:bg-accent"
                >
            ...
            <div className="hidden w-64 shrink-0 rounded-2xl border border-border bg-background/55 p-5 backdrop-blur-xl lg:block">
              ...
              <div className="mt-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Assistant engine connected</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="grid grid-cols-2 gap-3 lg:grid-cols-4 lg:gap-4">
          {stats.map(({ label, value, icon: Icon }) => (
            <article key={label} className="surface surface-hover rounded-2xl p-5 sm:p-6">
              ...
              <p className="mt-5 font-display text-2xl font-semibold tracking-tight text-foreground sm:text-3xl">
                {value}
              </p>
            </article>
          ))}
          ...
          <article className="surface overflow-hidden rounded-2xl">
            <div className="flex items-center justify-between border-b border-border px-5 py-5 sm:px-6">
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground sm:text-sm">Your latest files and their processing status.</p>
              </div>
              <button
                type="button"
                onClick={() => navigate("/documents")}
                className="rounded-lg border border-border px-3 py-2 text-xs font-semibold text-muted-foreground transition hover:bg-accent hover:text-foreground"
              >
                View all
              </button>
            </div>
            ...
            <div className="p-4 sm:p-5">
              {loading ? (
                ...
                <div className="rounded-xl border border-destructive/20 bg-destructive/10 p-5 text-sm text-destructive">
                  <p>{error}</p>
                  <button type="button" onClick={() => void loadDocuments()} className="mt-3 font-semibold underline">
                    Try again
                  </button>
                </div>
              ) : documents.length === 0 ? (
                <div className="rounded-xl border border-dashed border-border bg-muted/20 px-6 py-12 text-center">
                  ...
                  <p className="mx-auto mt-1 max-w-sm text-xs leading-5 text-muted-foreground sm:text-sm">
                    Upload your first document and start building your AI knowledge base.
                  </p>
                  <button
                    type="button"
                    onClick={() => navigate("/documents")}
                    className="mt-4 inline-flex h-9 items-center gap-2 rounded-lg bg-primary px-3.5 text-xs font-semibold text-primary-foreground"
                  >
                    <UploadCloud size={15} />
                    Upload document
                  </button>
                    ...
                    <div key={document.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-0.5 text-xs text-muted-foreground">
                          {document.file_type?.toUpperCase() || "FILE"} · {formatSize(document.file_size)}
                        </p>
                      </div>
          ...
          <article className="surface rounded-2xl p-5 sm:p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Jump straight into your workflow.</p>
              </div>
            </div>
...
function QuickAction({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="group flex w-full items-center gap-3 rounded-xl border border-transparent p-3 text-left transition hover:border-border hover:bg-muted/50"
    >
      ...
      <ArrowRight size={15} className="text-muted-foreground transition group-hover:translate-x-0.5 group-hover:text-foreground" />
    </button>
  );
}
