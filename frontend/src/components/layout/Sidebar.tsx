...
function Sidebar() {
  return (
    <aside
      className="flex h-full min-h-0 flex-col overflow-y-auto bg-card/90"
      aria-label="Sidebar"
    >
      <header className="flex-shrink-0 border-b border-border/70 px-5 py-6">
        <NavLink
          to="/dashboard"
          className="group flex items-center gap-3 rounded-2xl p-2 transition hover:bg-accent/60 focus:outline-none focus:ring-2 focus:ring-primary"
          aria-label="AIDoc dashboard"
        >
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-[9px] font-semibold uppercase tracking-[0.3em] text-muted-foreground">
          ...
          {MENU_ITEMS.map(({ name, path, icon: Icon }) => (
            <li key={path}>
              <NavLink
                ...
                className={({ isActive }) =>
                  [
                    "group relative flex items-center gap-3 rounded-xl px-3.5 py-3 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary",
                    isActive
              ...
              >
                {({ isActive }) => (
                  <>
                    {isActive && (
                      <span className="absolute left-0 top-1/2 h-6 w-0.5 -translate-y-1/2 rounded-full bg-primary-foreground/90" />
                    )}
                    <Icon
                      size={19}
      ...
      <footer className="flex-shrink-0 border-t border-border/70 p-4">
        <section className="rounded-2xl border border-border bg-muted/45 p-4">
          ...
          <p className="mt-3 text-xs leading-5 text-muted-foreground">
            Upload documents, search your knowledge base, and chat with your AI assistant.
          </p>
        </section>
      </footer>
    </aside>
